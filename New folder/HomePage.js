var HomePage = function() {
    ...
    this.findPhones = function() {
        var text1;
        var unsorted = element.all(by.repeater('phone in phones | filter:query | orderBy:orderProp')).get(5).getText().then(function(text) {
            console.log(text);
            text1 = text;
        }).then(function(){
        console.log(text1);
    ...
    this.sortAlpha = function() {
        var names = [];
         element.all(by.repeater('phone in phones | filter:query | orderBy:orderProp')).then(function (){
                names.sort();
         });
        return names;
    };
    this.sortM = function(str) {
